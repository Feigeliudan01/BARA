...
# Prediction results for fsva exact
...
normalization <- list(
  fit = function(x, ref, ...){
    ...
    y <- as.factor(as.numeric(vars$y))
    fit <- bapred::svaba(x = x, 
                         y = y, 
                         batch = batch, algorithm = 'exact')
    return(list(
      x_train = fit$xadj,
      fit = fit,
      ref = ref
  ...
  transform = function(fit, x_test, ref){
    x_test <- bapred::svabaaddon(params = fit$fit,
                                 x = x_test)
...
saveRDS(object = svm_full, 
        file = 'results/assess_normalization/fsva_exact/svm_full.RDS')
saveRDS(object = svm_small, 
        file = 'results/assess_normalization/fsva_exact/svm_small.RDS')
saveRDS(object = rf_full, 
        file = 'results/assess_normalization/fsva_exact/rf_full.RDS')
saveRDS(object = rf_small, 
        file = 'results/assess_normalization/fsva_exact/rf_small.RDS')
saveRDS(object = knn_full, 
        file = 'results/assess_normalization/fsva_exact/knn_full.RDS')
saveRDS(object = knn_small, 
        file = 'results/assess_normalization/fsva_exact/knn_small.RDS')